...
sim_Res1=foreach(a=icount(B),.combine = cbind,.inorder = FALSE,.packages = c("dqrng","glmnet","abind","foreach","iterators","HDSpectralAnalysis","glassoFast"),.errorhandling = "stop",.export = "q") %dopar% {
  ...
  erg_t_lambda=sapply(lambda,function(lambda_s)
  {
  
  Spec1=HDSpec::spec_inv_freq(Z = Z,Frequencies = FourierFrequencies_h1,lambda = lambda_s,Kernel = Kernel,M = M1,M_corr = M0[2])
  Spec1a=Spec1$spec*as.numeric(abs(Spec1$spec)>sapply(Spec1$lambdas,function(i) matrix(i,p,p),simplify = "array"))
  
  Spec_VAR_esta=spec_inv_preWhiteningVAR_freq(X_t = X_t,lambda=lambda_s,Kernel = Kernel,M = M2,VAR_order = VAR_order,Frequencies = FourierFrequencies_h2,method = "LASSO",crit="bic_m",Check_pos_def = T,M_corr = M0[2],mbic2_c =log(p),gamma=log(log(p)))
  ...
  tmp=list(FDR=erg_t_lambda[1,,,,],
           POWER=erg_t_lambda[2,,,,],
           Test=NULL,
           M1=M1,
           M2=M2)
}
...
erg_POWER_m=apply(erg_POWER,1:4,function(x) mean(unlist(x)))
erg_FDR_m=apply(erg_FDR,1:4,function(x) mean(unlist(x)))



erg_POWER_sd=apply(erg_POWER,1:4,function(x) sd(unlist(x)))
erg_FDR_sd=apply(erg_FDR,1:4,function(x) sd(unlist(x)))


save(A,B_m,Sigma,erg_M2,erg_M2a,True_R,erg_POWER_m,erg_FDR_m,erg_FDR,erg_POWER,erg_FDR_sd,erg_POWER_sd,file=sprintf("Spec_FDR_Lambda_Seq_n=%i_M1=ADAP_VAR_p=%i_d1=%0.2f_d2=%0.2f_%s",n,VAR_order,Delta1,Delta2,file))
